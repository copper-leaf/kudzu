...
class ParserComparisonTestsParsus : StringSpec({
    // this test takes a long time, so only run it as-needed and not part of normal development cycles
    "parsusSimpleExpression".config(enabled = ParserComparisonTests.enabled) {

        performanceTest(10_000, 1000) {
            val computedValue = ParsusExprCalculator.parseEntireOrThrow(ParserComparisonTests.simpleExpression)
        }
    }

    // this test takes a long time, so only run it as-needed and not part of normal development cycles
    "parsusDeeplyNestedExpression".config(enabled = ParserComparisonTests.enabled) {
        performanceTest(1, 0) {
            val computedValue = ParsusExprCalculator.parseEntireOrThrow(ParserComparisonTests.deeplyNestedExpression)
        }
    }
})
