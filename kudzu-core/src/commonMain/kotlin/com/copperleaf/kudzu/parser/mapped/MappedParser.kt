 ...
 */
public class MappedParser<ParserNodeType : Node, T>(
    public val parser: Parser<ParserNodeType>,
    public val remapErrors: RemapperFn = { _, e -> e },
    public val mapperFunction: ParserContext.(ParserNodeType) -> T,
) : Parser<ValueNode<T>> {
    override fun predict(input: ParserContext): Boolean {
        return parser.predict(input)
    }

    override val parse: ParseFunction<ValueNode<T>> = runParser { input ->
