...
class AtLeastParser(
    ...
    override fun parse(input: ParserContext): Pair<Node, ParserContext> {
        ...
        if (nodeList.size < minSize) throw ParserException(
            "must have at least $minSize ${if (minSize == 1) "match" else "matches"}",
            this,
            input
        )
...
class TimesParser(
    ...
    override fun parse(input: ParserContext): Pair<Node, ParserContext> {
        ...
        for (i in 0 until times) {
            if (!parser.predict(remaining)) throw ParserException(
                "unexpected end of input after $i iterations, expected $times",
                this,
                remaining
            )
            next = parser.parse(remaining)
