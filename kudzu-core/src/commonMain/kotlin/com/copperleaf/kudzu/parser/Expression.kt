    ...
    private fun setupExpressionParserWithFixedLevels(termParser: Parser, operators: List<Operator>): Parser {
        ...
        if (nonUniqueOperators.isNotEmpty()) throw IllegalArgumentException(
            "All operators must have unique names!\n" +
                "non-unique operator counts: " +
                nonUniqueOperators.entries.joinToString { "['${it.key}' -> ${it.value}]" }
        )
...
private data class OperatorLevel(
    val precedence: Int,
    val associativity: Int
) : Comparable<OperatorLevel> {
...
class PrefixEvaluableOperator<T : ExpressionContext<U>, U>(
    op: PrefixOperator,
...
class PostfixEvaluableOperator<T : ExpressionContext<U>, U>(
    op: PostfixOperator,
...
class InfixrEvaluableOperator<T : ExpressionContext<U>, U>(
    op: InfixrOperator,
...
class InfixEvaluableOperator<T : ExpressionContext<U>, U>(
    op: InfixOperator,
    val eval: (cxt: T, lhs: U, rhs: U) -> U
) : EvaluableOperator<T, U>(op) {
    constructor(
...
) : Visitor<T>({ it is NonTerminalNode }, "expressionRoot") {
